...
Cursorの左下に現在のブランチ名が表示されています。クリックすると、新しいブランチを作成できます。

**コマンドで行う場合**：

```bash
git checkout -b feature/add-edit-function
```

`feature/add-edit-function`の部分は、ブランチの名前です。何をするブランチかが分かる名前をつけましょう。

**ブランチ名の例**：

- `feature/add-edit-function`：編集機能を追加
- `fix/bug-in-sidebar`：サイドバーのバグを修正
- `update/readme`：READMEを更新

### ブランチを切り替える

Cursorの左下に現在のブランチ名が表示されています。クリックすると、ブランチの切り替えや作成ができます。

**コマンドで行う場合**：

```bash
git checkout feature/add-edit-function
```

### ブランチをプッシュする

新しいブランチで作業した場合、そのブランチをリモートに送信します。
